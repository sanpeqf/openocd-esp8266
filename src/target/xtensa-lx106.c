                  ...
                  uint8_t *buffer)
{
    ...
    */
    while(count > 255) {
        LOG_DEBUG("%s: splitting read from " TARGET_ADDR_FMT " size %d count 255",__func__,
              address,size);
        res = xtensa_read_memory_inner(target, address, size, 255, buffer);
        if(res != ERROR_OK) {
            LOG_ERROR("%s: inner read failed at address " TARGET_ADDR_FMT, __func__, address);
            return res;
        }
        count -= 255;
        address += (255 * size);
    ...
    if(res != ERROR_OK) {
        LOG_ERROR("%s: final read failed at address " TARGET_ADDR_FMT, __func__, address);
    }
                   ...
                   const uint8_t *buffer)
{
    ...
    */
    while(count > 255) {
        ...
        if(res != ERROR_OK) {
            LOG_ERROR("%s: inner write failed at address " TARGET_ADDR_FMT, __func__, address);
    ...
    if(res != ERROR_OK) {
        LOG_ERROR("%s: final write failed at address " TARGET_ADDR_FMT, __func__, address);
    }
...
static int xtensa_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
    ...
    if (breakpoint->type == BKPT_SOFT) {
        ...
        LOG_WARNING("Cannot set a software breakpoint at 0x%x. Trying a hardware one instead...", (uint32_t)breakpoint->address);
    }
...
static const struct command_registration xtensa_commands[] = {
    {
...
struct target_type xtensa_lx106_target = {
    .name = "xtensa-lx106",
